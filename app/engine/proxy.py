"""Proxy decision layer for LegadoHub fetcher."""
...
class ProxyConfig:
    ...
    def from_dict(cls, data: dict) -> "ProxyConfig":
        return cls(
            ...
            failure_error_keywords=data.get("failure_error_keywords", ["timeout", "connection", "reset", "forbidden", "captcha", "blocked"]),
        )
...
def should_retry_with_proxy(error: Exception, proxy_config: ProxyConfig) -> bool:
    """Determine if a failed request should be retried through proxy."""
    ...
    for code in proxy_config.failure_status_codes:
        if str(code) in error_str:
            return True

    # Check error keywords
    for keyword in proxy_config.failure_error_keywords:
