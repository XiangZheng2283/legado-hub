...
class Cache:
    ...
    def get_source_runtime_state(self, source_id: str) -> dict | None:
        with self._conn() as conn:
            row = conn.execute(
                "SELECT proxy_mode, proxy_status, last_direct_error, last_proxy_error, last_success_via_proxy, updated_at FROM source_runtime_state WHERE source_id=?",
                (source_id,),
            ).fetchone()
        if row:
            return {
    ...
    def set_source_runtime_state(
        ...
        with self._conn() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO source_runtime_state (source_id, proxy_mode, proxy_status, last_direct_error, last_proxy_error, last_success_via_proxy, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (source_id, proxy_mode, proxy_status, last_direct_error, last_proxy_error, int(last_success_via_proxy), self._now()),
            )
            conn.commit()
