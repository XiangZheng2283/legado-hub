...
echo Use the reader console on port 8765 for subscriptions and the admin console on port 8766 for management.
echo.

echo Checking for an existing LegadoHub server on ports 8765 and 8766...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference = 'SilentlyContinue'; function Test-LegadoHubPort($port) { try { $entry = Invoke-RestMethod -Uri ('http://127.0.0.1:' + $port + '/api/auth/entrypoint') -TimeoutSec 2; return $entry.entrypoint -in @('public', 'admin', 'combined') } catch { return $false } }; function PortsClosed { return -not (Get-NetTCPConnection -LocalPort 8765,8766 -State Listen) }; function Stop-LegadoHubListener($listenerPid) { Write-Host ('Stopping existing LegadoHub server PID ' + $listenerPid); if (Get-Process -Id $listenerPid) { Stop-Process -Id $listenerPid -Force; if (-not (PortsClosed)) { & taskkill /PID $listenerPid /T /F | Out-Null } } else { Write-Host ('PID ' + $listenerPid + ' is not exposed by Windows process list.') }; for ($i = 0; $i -lt 20; $i++) { Start-Sleep -Milliseconds 250; if (PortsClosed) { return $true } }; return $false }; $listeners = @(Get-NetTCPConnection -LocalPort 8765,8766 -State Listen); $handled = @{}; foreach ($listener in $listeners) { $listenerPid = $listener.OwningProcess; if ($handled.ContainsKey($listenerPid)) { continue }; $handled[$listenerPid] = $true; $proc = Get-CimInstance Win32_Process -Filter ('ProcessId=' + $listenerPid); $isLegadoHub = $false; if ($proc -and (($proc.CommandLine -like '*app.server*') -or ($proc.CommandLine -like '*uvicorn*app.main:app*'))) { $isLegadoHub = $true } else { $isLegadoHub = (Test-LegadoHubPort 8765) -or (Test-LegadoHubPort 8766) }; if ($isLegadoHub) { if (Stop-LegadoHubListener $listenerPid) { continue }; if ((Test-LegadoHubPort 8765) -and (Test-LegadoHubPort 8766)) { Write-Host 'Existing LegadoHub is still reachable; reusing it.'; exit 3 }; Write-Host ('Existing LegadoHub was detected, but its ports did not close after stopping PID ' + $listenerPid); exit 2 }; Write-Host ('A LegadoHub port is used by another process PID ' + $listenerPid + ': ' + $proc.CommandLine); exit 2 }"
if errorlevel 3 (
    echo LegadoHub is already running on ports 8765 and 8766. Use the URLs above.
    exit /b 0
)
if errorlevel 2 (
    echo Port 8765 or 8766 is occupied by a non-LegadoHub process. Please close it manually or change the port.
    pause
    exit /b 1
)
