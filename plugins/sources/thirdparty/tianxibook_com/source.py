"""Tianxibook (新天禧小说) source plugin."""
...
class Source:
    ...
    base_url = "https://www.tianxibook.com"
    ...
    async def search(self, ctx, keyword: str, page: int):
        ...
        try:
            html = await ctx.access.http.fetch_text(
                f"https://www.sososhu.com/?q={keyword}&site=xtxxs"
            )
            results = ctx.select(html, "body > div.wrap > div > div > div")
            for div in results:
                dl = ctx.select(div, "dl")
                if not dl:
                    continue
                name_a = ctx.select(dl[0], "dt > a")
                if not name_a:
                    continue
                name = ctx.clean_text(name_a[0].text_content())
                href = name_a[0].get("href", "")
                author = ctx.clean_text(ctx.text(dl[0], "dt > span"))
                intro = ctx.clean_text(ctx.text(dl[0], "dd"))
                items.append({
                    "sourceId": self.id,
                    "name": name,
                    "author": author,
            ...
            search_error = exc
            ctx.trace("search_error", url="https://www.sososhu.com/", message=str(exc))
        if not items:
    ...
    async def toc(self, ctx, toc_url: str):
        ...
        for _ in range(200):
            ...
            for a in links:
                ...
                new_count += 1
            next_href = ctx.attr(html, "#indexselect > option:last-child", "value")
    ...
    def _clean_chapter_content(self, html: str) -> str:
        ...
        for div in soup.find_all("div"):
            ...
            if len(text) < 80 and any(kw in text for kw in ["广告", "声明", "本章结束", "返回目录", "加入书签", "推荐", "最新网址", "章节内容缺失", "章节不存在", "首发网址", "一秒记住", "新天禧"]):
                div.decompose()
    ...
    async def chapter(self, ctx, chapter_url: str):
        ...
        while current_url and len(parts) < 10:
            ...
            if not title:
                title = ctx.text(html, "#wrapper > article > h1") or ctx.text(html, "h1")
            content_html = ctx.html(html, "#booktxt") or ctx.html(html, "#content")
            content = self._clean_chapter_content(content_html)
            if content:
                parts.append(content)
            next_href = ctx.attr(html, "#wrapper > article > div.bottem1 > a:nth-child(3)", "href")
