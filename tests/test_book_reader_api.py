...
def test_get_book_not_found():
    response = client.get("/api/admin/books/fake-book")
...
def test_get_chapter_invalid():
    response = client.get("/api/admin/chapter/invalid")
    assert response.status_code == 200
    data = response.json()
...
def test_chapter_navigation():
    response = client.get("/api/admin/books/fake/chapters/fake-chapter/navigation")
    assert response.status_code == 200
    data = response.json()
    assert "prev" in data
    assert "next" in data
...
def test_chapter_fallback():
    response = client.get("/api/admin/chapter/fake/fallback")
    assert response.status_code == 200
    data = response.json()
