...
def test_source_comment_mentions_legado_browser_challenge_api() -> None:
    source = generate_aggregate_source()[0]

    assert "/api/legado/browser-challenges/{session_id}/cookies" in source["bookSourceComment"]
    assert "/api/legado/browser-challenges/{session_id}/browser/open" in source["bookSourceComment"]
    assert "/api/legado/browser-challenges/{session_id}/retry-live-check" in source["bookSourceComment"]
    assert "debug.browserChallenges" in source["bookSourceComment"]
