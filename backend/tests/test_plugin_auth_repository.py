...
@pytest.mark.asyncio
async def test_context_browser_fetch_cookies_are_persisted_to_db_for_unknown_plugin(tmp_path):
    repo = PluginAuthRepository(tmp_path / "auth.db")
    ...
    class FakeAccessBridge:
        async def fetch(self, request):
            return AccessFetchResult(
                ...
                html="<html>ok</html>",
                cookies=[{"domain": ".legacytest.com", "name": "cf_clearance", "value": "ok"}],
            )

    ctx = PluginContext(
        fetcher=Fetcher(),
        plugin_id="legacy_test_plugin",
        auth_repository=repo,
        access_bridge=FakeAccessBridge(),
    )

    text = await ctx.access.browser.fetch_text("https://www.legacytest.com/")

    assert text == "<html>ok</html>"
    assert repo.get_cookies("legacy_test_plugin") == {"legacytest.com": {"cf_clearance": "ok"}}


@pytest.mark.asyncio
async def test_context_http_set_cookie_domain_is_persisted_to_db_for_unknown_plugin(tmp_path, monkeypatch):
    repo = PluginAuthRepository(tmp_path / "auth.db")
    ...
    async def fake_request(self, method, url, **kwargs):
        ...
        return httpx.Response(
            200,
            text="<html>ok</html>",
            headers={"set-cookie": "cf_clearance=ok; Domain=.legacytest.com; Path=/"},
            request=request,
        )

    monkeypatch.setattr("httpx.AsyncClient.request", fake_request)
    ctx = PluginContext(fetcher=Fetcher(), plugin_id="legacy_test_plugin", auth_repository=repo)

    text = await ctx.access.http.fetch_text("https://www.legacytest.com/newhot_0_1_1.htm")
