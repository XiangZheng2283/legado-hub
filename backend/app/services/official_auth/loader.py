"""Private plugin package loader for official sources.

Discovers and loads private packages under:
...
class PrivatePluginLoader:
    ...
    def _private_dir(self, plugin_id: str) -> Path | None:
        """Resolve private directory for a plugin."""
        # Try official/ first, then any subdirectory
        candidates = [
            self.BASE_DIR / "official" / plugin_id / "private",
            self.BASE_DIR / plugin_id / "private",
        ]
        for p in candidates:
    ...
    def _load_module(self, private_dir: Path, module_name: str) -> ModuleType | None:
        """Dynamically load a module from private directory."""
        file_path = private_dir / f"{module_name}.py"
        if not file_path.exists():
            return None

        # Use unique module name to avoid conflicts
        unique_name = f"_private_{private_dir.parent.name}_{module_name}"
    ...
    def load(self, plugin_id: str) -> dict[str, Any]:
        ...
        private_dir = self._private_dir(plugin_id)
        if not private_dir:
        ...
        if manifest.phone_auth:
            mod = self._load_module(private_dir, "auth_api")
            if mod:
        ...
        if manifest.cookie_auth:
            mod = self._load_module(private_dir, "cookie_auth")
            if mod:
                cookie_auth = self._resolve_contract(mod, CookieAuthContract)
                result["cookieAuth"] = cookie_auth

        if manifest.reviews:
            mod = self._load_module(private_dir, "reviews")
            if mod:
    ...
    def has_private(self, plugin_id: str) -> bool:
        return bool(self._private_dir(plugin_id))
