...
LegadoHub is used by a local power user who maintains self-owned novel source plugins on a Windows machine and accesses the service from Reading/Legado on phone or LAN devices. The primary context is operational: inspecting plugin health, debugging parser failures, tuning proxy behavior, validating search/detail/toc/content flows, and keeping a useful source pool available without depending on public Reading source rules.

## Product Purpose

LegadoHub is a local aggregation middleware and Python source-plugin runtime for Reading/Legado. It exposes one importable aggregate source to the Reading app while the local service handles search, detail, TOC, chapter content, caching, update tracking, source-plugin governance, proxy fallback, and later AI-assisted plugin creation and repair. Success means a user can import one source, search and read reliably, see why a plugin failed, and adapt broken or restricted sites without leaving the web backend.
