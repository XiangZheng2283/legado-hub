...
The aggregate source manifest remains anonymously importable. Search, book detail, TOC, chapter, review, subscription, and all management APIs require an authenticated identity. Reading search combines `visible` shared books with enabled third-party plugin results through the existing search coordinator. Direct reads are limited to published aggregate IDs or enabled non-official plugins whose decoded URLs stay inside the plugin-declared domains; official, disabled, unknown, and off-domain IDs remain blocked. Reading must not mutate subscriptions implicitly, enqueue aggregate work, or receive official-source credentials.
