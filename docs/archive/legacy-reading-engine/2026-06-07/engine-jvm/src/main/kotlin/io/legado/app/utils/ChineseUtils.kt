...
object ChineseUtils {
    ...
    fun preLoad(async: Boolean, vararg transType: TransType) {
        QuickChineseUtils.preLoad(async, *transType)
    }

    fun unLoad(vararg transType: TransType) {
        QuickChineseUtils.unLoad(*transType)
    }

    private fun fixT2sDict() {
        fixed = true
        QuickChineseUtils.loadExcludeDict(
