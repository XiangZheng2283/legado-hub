...
> **Superseded on 2026-06-07:** This roadmap is now a historical reference for the Reading-kernel-port direction. The active restart direction is `docs/architecture/plugin-source-runtime-restart-plan.md`: LegadoHub keeps the aggregate Reading/Legado source provider and backend operations console, but moves internal source execution to self-maintained Python source plugins.
