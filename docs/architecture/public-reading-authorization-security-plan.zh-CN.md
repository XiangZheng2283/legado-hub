...
- 官方插件目录不进入 Docker 镜像，由部署方从宿主只读挂载并在部署后单独放入；镜像和构建上下文都不得携带官方插件源码或私有资源。
- 第三方插件随镜像发布；仅在需要独立更新时显式叠加独立的第三方插件 override 目录。空目录继续使用镜像插件，同 ID 的第三方插件以宿主版本为准，且 override 不得新增或覆盖官方源。
