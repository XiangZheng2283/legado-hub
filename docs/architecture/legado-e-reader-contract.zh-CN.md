...
5. 不按正文每段插入气泡。规则按阅读字号估算页面字符窗口，将同一窗口内的热门段落合并成一个右侧半透明“热评 N”入口，并尽量插入估算页首；`N` 按该页可定位条目的 `commentCount -> totalCommentCount -> hotCommentCount` 求和，不使用 `topReviews` 预览数量代替总数。
6. 页入口通过 `paragraphIds` 打开当前页热门评论集合；点击集合内评论再进入对应段评，该页面不显示作家说和本章说。
7. 章末追加起点式单行全宽“本章说 / N 条评论”入口，打开作家说、本章说、段评说及回复的现有 HTML 页面。
8. 页入口与章末入口在每次正文规则执行时读取 `getThemeMode`、`getThemeConfigMap` 和 `getReadBookConfigMap`，使用当前日间、夜间或墨水屏文字色生成 SVG；背景与描边通过透明度适配阅读背景。
9. 规则在客户端完成真实分页前运行，`RIGHT` 只能控制入口在当前正文位置右对齐，不能提供分页后的绝对定位。当前实现是“估算页首”，精确固定到每页右上角需要客户端提供页级浮层契约，禁止用负边距或伪绝对定位冒充。
10. `legado-E` 会执行当前章及相邻预加载章节的正文规则，但没有暴露“当前展示章/预加载章”标志。每次执行都必须由该章自己的 `contentUrl` 派生评论地址，禁止用全局状态猜测当前章；`/reviews` 摘要与热段预览会随正文预加载，完整段评、本章说后续页和回复只在用户点击后通过插件扩展分页加载。
...
- Android 真机上的导入、授权输入、估算页首布局、日夜主题切换、页热评/章末入口点击和弹窗手势仍属于发布前人工验收。
