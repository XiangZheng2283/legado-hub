...
5. 用户自动归档不能直接改变共享任务；共享书在明确完结且所有处理、预览复查或重试均进入终态后统一归档，并同时终止该书全部用户订阅活动。
...
| `archived` | `active` | 进入已完结视图，保留阅读与历史；共享书已归档时不得恢复订阅活动 |

Phase 1 不提供硬删除订阅接口。再次订阅已归档书等价于恢复为 `active`。最后一个用户归档后，共享书也不得自动删除；无人订阅仅作为管理员清理候选。

共享书进入归档终态时，该书全部 `active` 或 `paused` 用户订阅统一改为 `archived`，不再产生订阅活动；正文、阅读关系与历史继续保留。
...
- `archived`：管理员明确停止常规追更，或完结书在全部处理与预览复查进入终态后自动停止调度；内容继续保留。
...
- 删除：管理员确认后可级联删除 `active/paused/archived` 订阅；存在运行租约时先停止后续调度、请求当前任务退出并限时等待，仅在任务未能及时退出时返回可重试的 409。
...
8. 管理员删除存在有效订阅的共享书时级联移除订阅；存在运行租约时请求任务安全退出后继续删除，退出超时则返回可重试状态。
